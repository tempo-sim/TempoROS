...
"
    ...
    static bool Publish(UTempoROSNode* Node, const FString& Topic, const __MESSAGETYPE__& Message)
    {
...
"
DECLARE_DYNAMIC_DELEGATE_OneParam(FTempoROS__MESSAGETYPE__Received, __MESSAGETYPE__, Value);
"

BP_SUBSCRIPTION_TEMPLATE=\
"
    UFUNCTION(BlueprintCallable)
    static bool AddSubscription(UTempoROSNode* Node, const FString& Topic, const FTempoROS__MESSAGETYPE__Received& TempoROSMessageReceivedEvent)
    {
...
# for file in Public
# lines = file | grep TempoROS__BPSupport
# publishers = lines | grep TToROSConverter (remember file?)
# subscriptions = lines | grep TFromROSConverter (remember file?)
# end for loop
# if publishers > 0 || subscriptions > 0
# Use BP_LIBRARY_TEMPLATE to make file __ModuleName__BlueprintFunctionLibrary.h in Public
# (don't forget to replace __MODULENAME__ AND __MODULEAPI__
# for file in files with publishers or subscriptions
# open header
# add BP_INCLUDE_TEMPLATE for file at __TEMPOROSBP_INCLUDE__
# stick another __TEMPOROSBP_INCLUDE__ below
# for publisher in publishers:
# extract message type from publisher
# open header
# add BP_PUBLISHER_TEMPLATE for message type at __TEMPOROSBP_METHODS__
# stick another __TEMPOROSBP_METHODS__ below
# for subscription in subscriptions:
# extract message type from subscription
# open header
# add BP_DELEGATE_TEMPLATE for message type at __TEMPOROSBP_DELEGATES__
# stick another __TEMPOROSBP_METHODS__ below
# add BP_SUBSCRIPTION_TEMPLATE for message type at __TEMPOROSBP_METHODS__
# stick another __TEMPOROSBP_METHODS__ below
# remove all __TEMPOROSBP*s
